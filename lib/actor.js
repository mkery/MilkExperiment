...
class Actor {
  ...
  drawPoints(steps, lastPoint){
    if(lastPoint){
      ...
      humanLines.drawLine(this.ctx, currentPoint.x, currentPoint.y, Math.floor(steps[0].x), Math.floor(steps[0].y));
    }
    else {
  ...
  drawLine(line)
  {
    humanLines.drawLine(this.ctx, line.x1, line.y1, line.x2, line.y2);
  ...
  draw(){
    ...
    var polygons = $(this.oDOM).find("polygon, polyline, line")
    var self = this
