...
class Actor {
  ...
  drawPoints5(steps, scaleFactor = 1){
    ...
    {
      humanLines.drawLine(this.ctx, currentPoint.x, currentPoint.y, Math.floor(steps[i].x)*scaleFactor, Math.floor(steps[i].y)*scaleFactor);
      currentPoint.x = Math.floor(steps[i].x)*scaleFactor
      currentPoint.y = Math.floor(steps[i].y)*scaleFactor
    }
  ...
  async draw(x, y, scaleFactor, rotate){
    if(!this.run)
      return
    if(!x)
      x = this.posX
    ...
    if(self.loop && self.run === true)
      setTimeout(self.draw.bind(self, null, null, scaleFactor, rotate), 400)
  }
